...
export const getPant = (id, form, handleFileListChange) => {
  axios.get(API_PATH.pant + `/${id}`).then((res) => {
    let M, L, S, XL, XXL;
...
export const editPant = (id, pant, fileList, navigate, setLoad) => {
  ...
  axios
    .put(API_PATH.pant + `/${id}`, pant)
    .then((res) => {
      axios
        .post(API_PATH.pant + `/upload/${res.data}`, imgArrResult, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
