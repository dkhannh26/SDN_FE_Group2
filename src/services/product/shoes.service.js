...
export const getShoesCustomer = (
  ...
  axios.get(API_PATH.shoes + `/${id}`).then((res) => {
    console.log(res.data);
    setShoes(res.data);
    const images = res.data?.images;
    let imgArrResult = [];
    ...
    selectSize(
      Object.keys(res.data.size[0])[0],
      Object.values(res.data.size[0])[0]
    );
  });
};
...
export const editShoes = (id, shoes, fileList, navigate) => {
  let imgArrResult = [];
  if (fileList) {
    // xư lý các hình có sẵn trong antd để chuyển thành dạng file
  ...
  axios
    .put(API_PATH.shoes + `/${id}`, shoes)
    .then((res) => {
      axios
        .post(API_PATH.shoes + `/upload/${res.data}`, imgArrResult, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
