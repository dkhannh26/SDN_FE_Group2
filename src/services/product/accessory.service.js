...
export const getAccessoryCustomer = (
  id,
  setAccessory,
  setImages,
  setCanvas,
  ...
  axios.get(API_PATH.accessory + `/${id}`).then((res) => {
...
export const getListAccessoryIncrease = (setAccessories) => {
  axios
    .get(API_PATH.accessory + "/increase")
    .then((res) => {
      setAccessories(res.data.data);
    })
    .catch((error) => console.error(error));
...
export const getListAccessoryDecrease = (setAccessories) => {
  axios
    .get(API_PATH.accessory + "/decrease")
    .then((res) => {
      setAccessories(res.data.data);
    })
    .catch((error) => console.error(error));
...
export const createAccessory = (accessory, fileList, navigate) => {
  axios
    .post(API_PATH.accessory, accessory)
    .then((res) => {
...
export const editAccessory = (id, accessory, fileList, navigate) => {
  ...
  axios
    .put(API_PATH.accessory + `/${id}`, accessory)
    .then((res) => {
      axios
        .post(API_PATH.accessory + `/upload/${res.data}`, imgArrResult, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
