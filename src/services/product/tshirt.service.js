...
export const getTshirtCustomer = (
  id,
  setTshirt,
  setImages,
  setCanvas,
  ...
  axios.get(API_PATH.tshirt + `/${id}`).then((res) => {
...
export const editTshirt = (id, tshirt, fileList, navigate) => {
  ...
  axios
    .put(API_PATH.tshirt + `/${id}`, tshirt)
    .then((res) => {
      axios
        .post(API_PATH.tshirt + `/upload/${res.data}`, imgArrResult, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
